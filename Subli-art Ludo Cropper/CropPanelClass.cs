...
namespace Subli_art_Ludo_Cropper
{
    ...
    public class CropPanelClass
    {
        ...
        public CropPanelClass(CropperForm cropperForm, float slotRatio, PanelType type)
        {
            ...
            m_cropPanel.Height = m_cropperForm.ClientSize.Height - m_cropperForm.m_menuStrip.Height - m_cropperForm.CurrSlotsPanel.Height;
            ...
            m_arrowPb.Name = m_cropperForm.m_langDictionary["Arrow Text Create"];
            ...
            if (m_type != PanelType.Last)
            {
                m_arrowPb.Location = new Point(
                    ((m_cropperForm.CurrSlotsPanel.SlotCount + 1) * m_cropperForm.CurrSlotsPanel.SlotGap) + (m_cropperForm.CurrSlotsPanel.SlotCount * m_cropperForm.CurrSlotsPanel.SlotWidth),
                    m_cropPanel.Height - m_arrowPb.Height);
            }
            else
            {
                m_arrowPb.Location = new Point(
                    ((m_cropperForm.CurrSlotsPanel.SlotCount + 2) * m_cropperForm.CurrSlotsPanel.SlotGap) + ((m_cropperForm.CurrSlotsPanel.SlotCount + 1) * m_cropperForm.CurrSlotsPanel.SlotWidth),
                    m_cropPanel.Height - m_arrowPb.Height);
            }
        ...
        private void m_cropPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (m_currImage == null)
            {
                ...
                if(m_cropPanel.Name != "")
                {
                    // Set the font for the name drawing
                    Font nameFont = new Font("Arial", m_cropPanel.Height * 0.08f);
                    float stringHeight = e.Graphics.MeasureString(m_cropPanel.Name, nameFont).Height;

                    // Draw crop panel name
                    e.Graphics.DrawString(
                        m_cropPanel.Name, nameFont, 
                        new SolidBrush(Color.FromArgb(255, 220, 208, 192)), 
                        new PointF(0f, m_cropPanel.Height - stringHeight));
                }
            }
            else
            {
        ...
        {
            // Draw the crop tool
            e.Graphics.FillRectangle(new SolidBrush(m_cropperForm.CurrSlotsPanel.GetSlotColor()), m_cropRect);

            // Draw the image on the crop rectangle
            if(m_bitmap != null)
                e.Graphics.DrawImage(m_bitmap, m_cropRect);

            // Show crop in slot
            m_cropperForm.CurrSlotsPanel.CropToSlot(m_currImage, m_cropRect);
        }
        ...
        private void m_colorBut_Click(object sender, EventArgs e)
        {
            ...
            if (cd.ShowDialog() == DialogResult.OK)
            {
                ...
                m_cropperForm.CurrSlotsPanel.SetSlotColor(newColor);
        ...
        {
            // Disapprove the slot
            m_cropperForm.CurrSlotsPanel.DisapproveSlot();
            HideArrow();
        ...
        private void m_arrowTimer_Tick(object sender, EventArgs e)
        {
            ...
            g.DrawString(
                m_arrowPb.Name, new Font("Arial", m_arrowPb.Width * 0.06f), Brushes.White,
                new Rectangle((int)m_arrowPos.X, (int)m_arrowPos.Y, m_arrowPb.Width, (int)(m_arrowPb.Height * 0.3f)),
                stringFormat);
        ...
        public void DeleteImage()
        {
            ...
            m_cropperForm.CurrSlotsPanel.DeleteSlot();
            m_cropperForm.CurrSlotsPanel.DisapproveSlot();
            HideArrow();
        }
        ...
        public void DisplayImage(string imagePath)
        {
            ...
            m_cropperForm.CurrSlotsPanel.SetSlotColor(Color.FromArgb(100, 255, 0, 0));

            // Set the color dialog button back color
            Color slotColor = m_cropperForm.CurrSlotsPanel.GetSlotColor();
        ...
        {
            // Approve the slot
            m_cropperForm.CurrSlotsPanel.ApproveSlot();

            // If all slots are approved
            if (m_cropperForm.CurrSlotsPanel.AllApproved)
                // Show arrow
                ShowArrow();                           
        }
        ...
        public void ShowArrow()
        {
            ...
            {                
                ChangeArrowMode(RightButtonModes.Create);
        ...
        public void ChangeArrowMode(RightButtonModes mode)
        {
            switch (mode)
            {
                case RightButtonModes.Next:
                {
                    m_arrowPb.Name = m_cropperForm.m_langDictionary["Arrow Text Next"];
                    ChangeArrowColor(Color.Green);
                    break;
                }
                case RightButtonModes.Exit:
                {
                    m_arrowPb.Name = m_cropperForm.m_langDictionary["Arrow Text Exit"];
                    ChangeArrowColor(Color.Green);
                    break;
                }
                default:
                {
                    m_arrowPb.Name = m_cropperForm.m_langDictionary["Arrow Text Create"];
                    ChangeArrowColor(Color.Red);
                    break;
                }
            }
        }

        private void ChangeArrowColor(Color newArrowColor)
        {
            Color actulaColor;

            for (int i = 0; i < m_arrowBmp.Width; i++)
            {
                for (int j = 0; j < m_arrowBmp.Height; j++)
                {
                    //get the pixel from the scrBitmap image
                    actulaColor = m_arrowBmp.GetPixel(i, j);

                    // > 150 because.. Images edges can be of low pixel colr. if we set all pixel color to new then there will be no smoothness left.
                    if (actulaColor.A > 150)
                        m_arrowBmp.SetPixel(i, j, newArrowColor);
                    else
                        m_arrowBmp.SetPixel(i, j, actulaColor);
                }
            }
        }
        #endregion
        ...
        public string Name
        {
            get { return m_cropPanel.Name; }
            set { m_cropPanel.Name = value; }
        }

        public int Width
        {
            get { return m_cropPanel.Width; }
        }
        ...
        {
            get { return m_cropPanel.Location; }
            set { m_cropPanel.Location = value; }
        }
        ...
        public Bitmap PanelBitmap
        {
            get
            {
                Bitmap panelBitmap = new Bitmap(m_cropPanel.Width, m_cropPanel.Height);
                m_cropPanel.DrawToBitmap(panelBitmap, new Rectangle(0, 0, m_cropPanel.Width, m_cropPanel.Height));
                return panelBitmap;
            }
        }
