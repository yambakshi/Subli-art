...
namespace Subli_art_Ludo_Cropper
{
    public class ImagesLibraryClass
    {
        ...
        private void ThumbnailClick(object sender, EventArgs e)
        {
            ...
            m_cropperForm.CurrCropPanel.DisplayImage(((PictureBox)sender).Name);
            ...
            else 
            {
                ...
                if (((string[])result[1]).Length > 0)
                    ...
                    if (!((bool)result[0]))
                        ...
                        if (m_cropperForm.CurrSlotsPanel.AllEmpty)
                        {
                            // Load the first image from the image list to the current slot
                            m_cropperForm.CurrCropPanel.DisplayImage(((string[])result[1])[0]);
                        }
            }
        }
        #endregion
